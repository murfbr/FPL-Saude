...
async function fetchAppointments(startStr: string, endStr: string, filters?: KpiFilters) {
  ...
  const qParts: any[] = []
  if (filters?.professionalId && filters.professionalId !== 'all') qParts.push(where('professional_id', '==', filters.professionalId))
  // Em NoSQL, o ideal é não fazer ranges gigantes na UI. Para o dashboard vamos carregar o bloco e filtrar the rest na memória
  const q = query(apptsRef, ...qParts) // Removido restrições estritas de the range for this MVP bypass missing indexes
  ...
  for (const d of snap.docs) {
    const data = d.data()
    // Filtro In Memory para evitar Missing Indexes nos relatórios
