...
export async function bookAppointment(
  ...
  try {
    ...
    const [clientSnap, profSnap, serviceSnap] = await Promise.all([
      ...
      getDoc(
        doc(db, 'companies', getCompanyId(), 'professionals', professionalId),
      ),
      ...
      schedules: {
        ...
        end_time: new Date(
          new Date(startTime).getTime() +
            (serviceData?.duration_minutes || 60) * 60000,
        ).toISOString(),
      },
...
export async function bookRecurringAppointments(
  ...
  try {
    if (!daysOfWeek || daysOfWeek.length === 0) {
      return {
        error: new Error('Nenhum dia da semana selecionado para recorrência.'),
      }
    }
      ...
      for (let w = 0; w < weeks; w++) {
        ...
        const appInfo = {
          ...
          schedules: {
            ...
            end_time: new Date(
              occurrenceDate.getTime() + duration * 60000,
            ).toISOString(),
          },
          // Desnormalizado para sumários de parceria
          partnership_id: clientData?.partnership_id || null,
...
export async function rescheduleAppointment(
  ...
  try {
    ...
    const updates: any = {
      ...
      'schedules.end_time': new Date(
        new Date(newStartTime).getTime() + duration * 60000,
      ).toISOString(),
    }
...
/**
 * Muda o status via Cloud Function (transação no servidor): débito/estorno de
 * sessão de pacote, registro financeiro com ID determinístico e billing_type
 * acontecem com privilégio do servidor — o profissional não precisa (nem tem)
 * acesso de escrita a financial_records, e dois cliques simultâneos não
 * duplicam receita.
 */
export async function updateAppointmentStatus(
  ...
  try {
    await callSetAppointmentStatus({
      appointmentId,
      status,
      allowExhaustedPackageUse: options?.allowExhaustedPackageUse === true,
    })
    return { error: null }
  } catch (e) {
    const raw = e as { message?: string; details?: { code?: string } }
    const error = new Error(
      raw?.message || 'Falha ao atualizar o status.',
    ) as Error & {
      code?: string
    }
    // A dialog de cortesia depende deste código para oferecer a conclusão sem débito
    if (raw?.details?.code) error.code = raw.details.code
    return { error }
  }
}
...
export async function deleteAppointment(
  appointmentId: string,
): Promise<{ deletedIds?: string[]; error: any }> {
  try {
    ...
    if (appSnap.exists()) {
      ...
      if (isEvent) {
        ...
        const wasConsumingStatus =
          appData.status === 'completed' || appData.status === 'no_show'

        if (
          isPackage &&
          wasConsumingStatus &&
          appData.client_id &&
          appData.package_session_consumed !== false
        ) {
...
export async function deleteFutureAppointments(
  appointmentId: string,
): Promise<{ deletedIds?: string[]; error: any }> {
  try {
    const companyId = getCompanyId()
    ...
    const sourceStartTimeMs = sourceStartTimeStr
      ? new Date(sourceStartTimeStr).getTime()
      : 0
