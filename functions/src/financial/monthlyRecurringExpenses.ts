...
import { currentMonthKey, previousMonthKey } from '../shared/summaryCore'
 ...
 */
export const monthlyRecurringExpenses = onSchedule(
  {
    schedule: '0 4 1 * *',
    region: REGION,
  ...
  async () => {
    ...
    for (const companyRef of companies) {
      ...
      for (const docSnap of snap.docs) {
        ...
        const dueMonth = ((e.due_date as string) || '').slice(0, 7)
        if (dueMonth !== prevKey) continue

        const rootId = (e.recurrence_source_id as string) || docSnap.id
        ...
        const dueDay = (e.due_date as string).slice(8, 10)
    ...
    console.log(
      `[recurring-expenses] ${monthKey}: ${created} lançamento(s) gerado(s) em ${companies.length} empresa(s)`,
    )
  },
)
