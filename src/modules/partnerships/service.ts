...
export async function updatePartnership(
  ...
  try {
    const docRef = doc(db, 'companies', getCompanyId(), 'partnerships', id)
...
export async function getDiscountsForPartnership(
  ...
  try {
    const docRef = doc(db, 'companies', getCompanyId(), 'partnerships', partnershipId)
    ...
    const hydrated = await Promise.all(discounts.map(async (d: any) => {
      ...
      if (!isGlobal) {
        const sRef = doc(db, 'companies', getCompanyId(), 'services', d.service_id)
